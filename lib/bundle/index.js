...
module.exports = async ( opt ) => {
    ...
    if ( !DEV ) {
        switch ( BUNDLE_TYPE ) {
            case 'ios': await bundleIOS( DEFAULT_IOS_JS_PATH, DEFAULT_IOS_STATIC_PATH, 'true' );
                break;
            case 'android': await bundleAndroid( DEFAULT_ANDROID_JS_PATH, DEFAULT_ANDROID_STATIC_PATH, 'true' );
                break;
            default: await bundleIOS( DEFAULT_IOS_JS_PATH, DEFAULT_IOS_STATIC_PATH ); await bundleAndroid( DEFAULT_ANDROID_JS_PATH, DEFAULT_ANDROID_STATIC_PATH );
        }
    } else {
        ...
        const RELEASE_ANDROID_STATIC_PATH = path.join( CURR_VERSION_BUNDLE_ANDROID_DIR, '/res' );

        // 构建到专门的目录下，不直接到ios和android的目录
        await bundleIOS( RELEASE_IOS_JS_PATH, RELEASE_IOS_STATIC_PATH, 'false' );
        await bundleAndroid( RELEASE_ANDROID_JS_PATH, RELEASE_ANDROID_STATIC_PATH, 'false' );
        ...
        await syncFile( [
            ...
            {
                from: RELEASE_ANDROID_STATIC_PATH,
