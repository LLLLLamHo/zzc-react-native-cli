...
if [[ -s "index.android.js" ]]; then
  ENTRY_FILE=${1:-index.android.js}
fi
...
[ -z "$RESOURCE_NAME" ] && export RESOURCE_NAME="main"
...
echo "$FILE_NAME" > "$DEST/jsbundleName.txt"

echo "$MODULE_NAME" > "$DEST/moduleName.txt"


if [[ -z "$CODEPUSH" ]]; then
CODE_PUSH_FLAG=""
else
CODE_PUSH_FLAG="$CODEPUSH"
fi
echo "$CODE_PUSH_FLAG" > "$DEST/codePush.txt"
echo "==============================="
echo "$CODE_PUSH_FLAG"
echo "==============================="
...
BUNDLE_FILE="$DEST/$FILE_NAME.jsbundle"

"$NODE_BINARY" "--max_old_space_size=4096" $NODE_ARGS "$CLI_PATH" $BUNDLE_COMMAND \
  $CONFIG_ARG \
